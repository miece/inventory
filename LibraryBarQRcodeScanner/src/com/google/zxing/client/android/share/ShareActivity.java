...
/**
 * Barcode Scanner can share data like contacts and bookmarks by displaying a QR Code on screen, such that another user can scan the barcode with
 * their phone.
 * 
...
public final class ShareActivity extends Activity {
	...
	/**
	 * Takes a contact Uri and does the necessary database lookups to retrieve that person's info, then sends an Encode intent to render it as a QR
	 * Code.
	 * 
	 * @param contactUri
	...
	private void showContactAsBarcode(Uri contactUri) {
		...
		Cursor methodsCursor = resolver.query(ContactsContract.CommonDataKinds.StructuredPostal.CONTENT_URI, null,
				ContactsContract.CommonDataKinds.StructuredPostal.CONTACT_ID + '=' + id, null, null);
		if (methodsCursor != null) {
			try {
				if (methodsCursor.moveToNext()) {
					String data = methodsCursor.getString(methodsCursor

...
public final class CalendarResultHandler extends ResultHandler {
	...
	public void handleButtonPress(int index) {
		if (index == 0) {
			...
			addCalendarEvent(calendarResult.getSummary(), calendarResult.getStart(), calendarResult.isStartAllDay(), calendarResult.getEnd(),
					calendarResult.getLocation(), description, calendarResult.getAttendees());
		}
	}

	/**
	 * Sends an intent to create a new calendar event by prepopulating the Add Event UI. Older versions of the system have a bug where the event title
	...
	private static String format(boolean allDay, Date date) {
		...
		DateFormat format = allDay ? DateFormat.getDateInstance(DateFormat.MEDIUM) : DateFormat.getDateTimeInstance(DateFormat.MEDIUM,
				DateFormat.MEDIUM);
